...
pub fn get_smoothers_quad_to_square(quad: &[Vec3; 4], axis: Vec3) -> [Vec3; 4] {
    ...
    let rotated = corners.iter().enumerate().map(|(i, corner)| {
        Vectors::rotate_axis_angle(axis, -PI * (i as fxx) * 0.5, center - *corner)
    }).collect::<Vec<_>>();
    let average_rot = Vectors::average(&rotated);
    let deltas = rotated.iter().enumerate().map(|(i, rot)| {
        Vectors::rotate_axis_angle(axis, PI * (i as fxx) * 0.5, *rot - average_rot)
    }).collect::<Vec<_>>();
...
pub fn get_smoothers_quad_to_square_at_length(quad: &[Vec3; 4], axis: Vec3, length: fxx) -> [Vec3; 4] {
    let corners = quad.to_vec();
    let center = Vectors::average(&corners);
    ...
    let deltas = rotated.iter().enumerate().map(|(i, rot)| {
        Vectors::rotate_axis_angle(axis, PI * (i as fxx) * 0.5, *rot - average_rot_at_right_length)
    }).collect::<Vec<_>>();
...
#[cfg(test)]
mod tests {
    use crate::{kernel::{Vec3, vec3}, pts::Vectors};
    ...
    fn test_quad_smoothing() {
        let mut some_quad = [vec3(0.0,0.0,0.0), vec3(1.0,0.0,0.0), vec3(1.0,1.0,0.0), vec3(0.0,1.0,0.0)];

        assert_eq!(Vectors::average(&some_quad.to_vec()), Vec3::new(0.5, 0.5, 0.0));
        assert_eq!(Vectors::rotate_pt_axis_angle(vec3(0.5,0.5,0.0), Vec3::Z, 0.0, Vec3::ZERO), Vec3::ZERO);
        
        let smoothers = get_smoothers_quad_to_square(&some_quad, Vec3::Z);
        println!("should all be around zero: {smoothers:?}");
        
        // now mutate the quad
        some_quad[2] = Vectors::rotate_pt_axis_angle(vec3(0.5,0.5,0.0), Vec3::Z, 0.1, some_quad[2]);
        some_quad[3] = Vectors::rotate_pt_axis_angle(vec3(0.5,0.5,0.0), Vec3::Z, -0.1, some_quad[3]);
        println!();
